...
6. Re-source .zshrc
7. Generate SSH key + gh auth login  ← pivot point: git-over-ssh now works
8. Clone + install dotfiles  (overwrites temp .zshenv/.zshrc with permanent symlinks)
9. Install sman binary + rc + snippets  (requires SSH from step 7)
10. Clone envoy + generate completions
11. Clone ssh-dir → replace ~/.ssh
```
...
|----------|-----------------|---------|
| `__LOCAL_ROOT` | `$HOME/.local` | Arch-**independent** prefix (share, state) |
...
`.zshrc` handles interactive shell: defines `ml_*` / `mu_*` "module" functions that lazily add tool directories to PATH (conda, pixi, brew, cargo, ghcup, etc.), then calls `ml` (load all) or `ml_clean` (minimal, when `__CLEAN=1` is set). Also sets up starship, fzf, direnv.
